 ...
 * 
 *         This class will contain registered listener interfaces that get
 *         notified when a particular screen changes graphically.
 * 
 *         For each ScreenMonitor there is exactly one screen device being
 *         sampled for which there can be one or more listening components.
 * 
 *         Components will be notified when a segment of a screen changes. It is
 *         then the responsibility of the listening component to fetch from this
 *         class the data segment of interest.
 * 
 *         Should all listeners be terminated, then the Screen Monitor shall
 *         terminate and be ready for garbage collection.
 * 
 *         Let each screen be broken up into segmentWidth by segmentHeight pixel
 *         segments.
 * 
 *         Let each segment be assigned an ID from left to right, top down where
 *         the first tile is ID 0 Let segment ID -1 indicate the collection of
 *         segments as a whole (The entire screen)
 *
 */
...
public class Monitor extends Thread {
    ...
    public static final long NO_LISTENER_MS = 5000; // dummy value to allow
                                                    // things to settle into nop
                                                    // state
    private static final boolean OBEY_SPEED_LIMITS = (Boolean) SERVER_PROPERTIES.OBEY_SPEED_LIMITS
            .getValue();
    private static final long MIN_REFRESH_MS = (Long) SERVER_PROPERTIES.MIN_MONITOR_SCANNING_PERIOD
            .getValue();
    int screenX, screenY;
    ...
    public Monitor(int segmentWidth, int segmentHeight, DirectRobot dirbot,
    ...
    private void syncBounds() {
        ...
        if (lastWidth == null || lastWidth != screenWidth
                || lastHeight != screenHeight) {
            segInfo.loadConfig(screenWidth, screenHeight, segInfo.segmentWidth,
                    segInfo.segmentHeight);
            ...
            if (lastWidth != null) {
                ...
                StateMachine.handleServerEvent(clients,
                        SERVER_EVENT.SCREEN_RESIZED, screenWidth, screenHeight);
            }
        }
    }
    ...
    private void syncMouse() {
        ...
        if (mouseOnMyScreen) {
            if (mouseWasOnMyScreen == null || lastX != mouseLocation.x
                    || lastY != mouseLocation.y) {
                StateMachine.handleServerEvent(clients,
                        SERVER_EVENT.CURSOR_MOVE,
                        Integer.valueOf(mouseLocation.x),
                        Integer.valueOf(mouseLocation.y));
            }
    ...
    public void run() {
        ...
        GraphicsSegment.Synchronously refresh = new GraphicsSegment.Synchronously() {
            ...
            public Object run(GraphicsSegment receiver, int[] pixels,
                    Integer[] solidColorPtr) {
                boolean changed = copyIntArray(pixels, buffer, pixels.length,
                        solidColorPtr);
                if (changed) {
                    receiver.releaseJitCompressedEvent();
                }
            ...
            while (true) {
                ...
                try {
                    ...
                    for (int i = 0; i <= segInfo.maxSegmentID; i++) {
                        ...
                        dirbot.getRGBPixels(x, y, segmentDim[0], segmentDim[1],
                                buffer);
                    ...
                    if (newClients.size() > 0) {
                        FixedLengthBitSet tmp = new FixedLengthBitSet(
                                changedSegments.length, Boolean.TRUE);
    ...
    public void sendDisplayInitEvents(ClientHandler client) {
        Rectangle bounds = getScreenBounds();
        client.sendEvent(SERVER_EVENT.SCREEN_RESIZED, bounds.width,
                bounds.height);
        client.sendEvent(SERVER_EVENT.SCREEN_SEGMENT_SIZE_UPDATE,
                segInfo.segmentWidth, segInfo.segmentHeight);
        if (mouseOnMyScreen != null && mouseOnMyScreen) {
            client.sendEvent(SERVER_EVENT.CURSOR_MOVE,
                    Integer.valueOf(mouseLocation.x),
                    Integer.valueOf(mouseLocation.y));
        }
    }
    ...
    /**
     * Eratta: src array content MAY CHANGE as a result of this function! It is
     * faster to fill a buffer with a SOLID color and compare arrays using a
     * syscall than it is to iterate over the array and compare individual
     * elements with a sample color.
     * 
        ...
        if (cachedSolidColor != null && cachedSolidColor.length > 0) {
            ...
            if (cachedSolidColor[0] != null) {
                ...
                for (; i < length; i++) {
                    ...
                    cachedSolidColor[0] = (i == 0 && isOneColor(dst, 0, length)) ? dst[0]
                            : null;
                    return true;
                }
                return false;
            ...
            if (src[0] != dst[0]) {
                ...
                cachedSolidColor[0] = isOneColor(dst, 0, length) ? dst[0]
                        : null;
                return true;
            }
            ...
            for (; i < length; i++) {
                if (src[i] != dst[i]) {
                    ...
                    System.arraycopy(src, i, dst, i, length - i);
                    cachedSolidColor[0] = isOneColor(dst, i - 1, length) ? dst[0]
                            : null;
                    return true;
                }
                if (src[i] != solidColor) {
