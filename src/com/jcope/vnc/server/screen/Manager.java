...
public class Manager extends Thread {
            ...
            try {
                ...
                if (numMissing > 0) {
                    try {
                        ...
                        missingSet = tmpGraphicsDeviceSet.keySet().toArray(
                                missingSet);
                ...
                if (setChanged) {
                    sendToAll(SERVER_EVENT.NUM_SCREENS_CHANGED,
                            Integer.valueOf(graphicDevicesSet.size()));
                } else {
                    try {
                        sleep(refreshMS);
                    } catch (InterruptedException e) {
    ...
    private void createMonitorForGraphicsDevice(GraphicsDevice graphicsDevice,
            ArrayList<ClientHandler> registeredClients) {
        ...
        Monitor monitor = new Monitor(SEGMENT_WIDTH, SEGMENT_HEIGHT, dirbot,
                registeredClients);
        for (ClientHandler client : registeredClients) {
            client.bindMonitor(monitor);
    ...
    public void withLock(Runnable r, Object... args) {
        ...
        try {
            stagedArgs = args;
            synchronized (clientsPerGraphicsDevice) {
                synchronized (monitorForGraphicsDevice) {
                    r.run();
                }
            }
        } finally {
    ...
    private Runnable actionBind = new Runnable() {
        ...
        public void run() {
            ...
            actionUnbind.run();
            ArrayList<ClientHandler> registeredClients = clientsPerGraphicsDevice
                    .get(graphicsDevice);
            ...
            if (newMonitor) {
                createMonitorForGraphicsDevice(graphicsDevice,
                        registeredClients);
            }
        }
    ...
    private Runnable actionUnbind = new Runnable() {
        ...
        public void run() {
            ...
            for (Entry<GraphicsDevice, ArrayList<ClientHandler>> entry : clientsPerGraphicsDevice
                    .entrySet()) {
                ArrayList<ClientHandler> list = entry.getValue();
                if (list.contains(client)) {
                    GraphicsDevice graphicsDevice = entry.getKey();
                    Monitor monitor = monitorForGraphicsDevice
                            .get(graphicsDevice);
                    if (monitor != null) {
    ...
    private Runnable actionUnbindAll = new Runnable() {
        ...
        public void run() {
            ...
            ArrayList<ClientHandler> clientsToSignal = clientsPerGraphicsDevice
                    .remove(graphicsDevice);
            if (clientsToSignal != null) {
                try {
                    SERVER_EVENT event = SERVER_EVENT.SCREEN_GONE;
                    JitCompressedEvent jce = (clientsToSignal.size() > 1) ? JitCompressedEvent
                            .getInstance(event, (Object[]) null) : null;
                    try {
    ...
    private Runnable actionSendEventToAllExcept = new Runnable() {
        ...
        @Override
        public void run() {
            ClientHandler notThiz = stagedArgs[0] == null ? null
                    : ((ClientHandler) stagedArgs[0]);
            SERVER_EVENT evt = (SERVER_EVENT) stagedArgs[1];
            Object[] evtArgs = (Object[]) stagedArgs[2];
            JitCompressedEvent jce = JitCompressedEvent.getInstance(evt,
                    evtArgs);
            try {
                for (ArrayList<ClientHandler> clientList : clientsPerGraphicsDevice
                        .values()) {
    ...
    public boolean bind(ClientHandler client, GraphicsDevice graphicsDevice,
            ACCESS_MODE accessMode, String passwordHash) {
        boolean rval = false;

        if (VncServer.securityPolicy.checkAuth(graphicsDevice, accessMode,
                passwordHash)) {
    ...
    public void sendToAllExcept(ClientHandler notThiz, SERVER_EVENT evt,
            Object... args) {
