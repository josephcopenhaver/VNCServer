...
public class SecurityPolicy {
    private static final Object[][] xmlStruct = new Object[][] { {
            ...
            new Object[][] { {
                    ...
                    new String[] { "guid" },
                    new Object[][] { { "opening",
                            new String[] { "mode", "authToken" }, null } } } } } };

    private HashMap<String, HashMap<ACCESS_MODE, String>> selectableDevices = new HashMap<String, HashMap<ACCESS_MODE, String>>(
            1);
    ...
    public void readPolicy(File file) throws ParserConfigurationException,
        ...
        assert_(rootNode.getAttributes().getLength() == XMLTools
                .attrLen(xmlStruct[0][1]));
        ...
        for (Node deviceNode : XMLTools.children(rootNode)) {
            ...
            String guid = ((Element) deviceNode)
                    .getAttribute(((String[]) subStruct1[0][1])[0]);

            for (Node openingNode : XMLTools.children(deviceNode)) {
                assert_(openingNode.getNodeName().equals(subStruct2[0][0]));

                String accessModeStr = ((Element) openingNode)
                        .getAttribute(((String[]) subStruct2[0][1])[0]);
                String passwordHash = ((Element) openingNode)
                        .getAttribute(((String[]) subStruct2[0][1])[1]);
    ...
    public void writePolicy(File file) throws ParserConfigurationException,
            TransformerException, IOException {

        if (file.exists() && !file.delete()) {
            throw new IOException(String.format("Failed to delete file: %s",
        ...
        for (String guid : guids) {
            HashMap<ACCESS_MODE, String> accessModes = selectableDevices
                    .get(guid);

            Element whiteListedDevice = doc
                    .createElement((String) subStruct1[0][0]);
            whiteListedDevice.setAttribute(((String[]) subStruct1[0][1])[0],
                    guid);
            ...
            for (ACCESS_MODE accessMode : ACCESS_MODE.sorted()) {
                ...
                Element whiteListEntryPoint = doc
                        .createElement((String) subStruct2[0][0]);
                whiteListEntryPoint.setAttribute(
                        ((String[]) subStruct2[0][1])[0], mode);
                whiteListEntryPoint.setAttribute(
                        ((String[]) subStruct2[0][1])[1], authToken);
    ...
    public void whitelist(String deviceIDString, ACCESS_MODE accessMode,
        ...
        if (deviceIDString.equals(ALL_TOKEN) && ACCESS_MODE.ALL == accessMode
                && passwordHash.equals(ALL_TOKEN)) {
            clear();
            fullAccess = true;
        } else if (ACCESS_MODE.ALL == accessMode
                && passwordHash.equals(ALL_TOKEN)) {
        ...
        HashMap<ACCESS_MODE, String> availableModes = selectableDevices
                .get(deviceIDString);
    ...
    public GraphicsDevice[] getEnabledDevices(ACCESS_MODE opt_accessMode) {
        ...
        for (GraphicsDevice device : getScreenDevices()) {
            if ((availableModes = selectableDevices.get(device.getIDstring())) != null
                    && (opt_accessMode == null
                            || availableModes.get(ACCESS_MODE.ALL) != null || availableModes
                            .get(opt_accessMode) != null)) {
    ...
    public boolean checkAuth(GraphicsDevice device, ACCESS_MODE accessMode,
            String challengeHash) {
        return checkAuth(device.getIDstring(), accessMode, challengeHash);
    }

    private boolean checkAuth(String deviceIDString, ACCESS_MODE accessMode,
            String challengeHash) {
        boolean rval = false;
        String passwordHash;
        ...
        if (availableModes != null) {
            if ((passwordHash = availableModes.get(ACCESS_MODE.ALL)) != null
                    && (passwordHash.equals(ALL_TOKEN) || passwordHash
