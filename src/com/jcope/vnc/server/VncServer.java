...
public class VncServer implements Runnable {
    ...
    public VncServer(int serverPort, int listenBacklog,
            String serverBindAddress, byte[] ipSpec, byte[] ipMask)
            throws ParserConfigurationException, SAXException, IOException {
        if (serverBindAddress == null) {
            this.serverBindAddress = null;
        } else {
            this.serverBindAddress = null;
            if (serverBindAddress.matches("^\\s+")
                    || serverBindAddress.matches("\\s+$")) {
                serverBindAddress = serverBindAddress.trim();
            }

            if (serverBindAddress.matches(IPV_4_6_REGEX_STR)) {
                this.serverBindAddress = InetAddress
                        .getByName(serverBindAddress);
            } else {
                    ...
                    do {
                        for (InetAddress addr : InetAddress
                            ...
                            if (saddr.equals("localhost")
                                ...
                                LLog.w(String
                                        .format("Interface name format \"%s\" for address \"%s\" was not recognized as a usable IP",
                                                saddr, oldServerBindAddress));
                            }
                        }
                        if (serverBindAddress != null) {
                            break;
                        ...
                        InetAddress[] addrs = dns
                                .lookupAllHostAddr(oldServerBindAddress);
                        if (addrs.length == 0) {
                            LLog.w(String
                                    .format("Host \"%s\" is unknown by the DNS server!\nTrying DNS server again in 3 seconds...",
                                            oldServerBindAddress));
                            try {
                        ...
                        for (InetAddress addr : addrs) {
                            ...
                            if (saddr.equals("localhost")
                                ...
                                LLog.w(String
                                        .format("Interface name format \"%s\" for address \"%s\" from DNS was not recognized as a usable IP",
                                                saddr, oldServerBindAddress));
                            }
                        }
                    } while (false);
                    ...
                    if (serverBindAddress == null) {
                        ...
                        if (isLoopbackPossible) {
                            ...
                            LLog.w(String.format(
                                    "Using loopback for bind address :\"%s\"",
                                    oldServerBindAddress));
                        } else {
                            LLog.e(new RuntimeException(String.format(
                                    "Failed to get IP for :\"%s\"",
                                    oldServerBindAddress)), true, true);
                        }
                    }
                }
                this.serverBindAddress = InetAddress
        ...
        File file = new File(
                (String) SERVER_PROPERTIES.SERVER_SECURITY_POLICY.getValue());
