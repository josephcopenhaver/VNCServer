...
public final class DirectRobot {
    ...
    public DirectRobot(GraphicsDevice device) throws AWTException {
        ...
        try {
            method = peerClass.getDeclaredMethod("getRGBPixels",
        ...
        if (methodType < 0) {
            try {
                method = peerClass.getDeclaredMethod("getScreenPixels",
                        new Class<?>[] { Rectangle.class, int[].class });
                methodType = 1;
            } catch (SecurityException e) {
                exceptions.add(e);
            } catch (NoSuchMethodException e) {
        ...
        if (methodType < 0) {
            try {
                method = peerClass.getDeclaredMethod("getScreenPixels",
                        new Class<?>[] { Integer.TYPE, Rectangle.class,
                                int[].class });
                methodType = 2;
        ...
        if (methodType < 0) {
            try {
                ...
                method = peerClass.getDeclaredMethod(
                        "getRGBPixelsImpl",
                        new Class<?>[] {
        ...
        if (methodType >= 0 && method != null
                && (methodType <= 1 || methodParam != null)) {
            if (DEBUG)
                LLog.i(String.format("Method type = %d", methodType));
            ...
            if (DEBUG) {
                LLog.w(String
                        .format("WARNING: Failed to acquire direct method for grabbing pixels, please post this on the main thread!\n\n%s\n\n",
    ...
    public static GraphicsDevice getMouseInfo(Point point) {
        ...
        if (!hasMouseInfoPeer) {
            ...
            try {
                Toolkit toolkit = Toolkit.getDefaultToolkit();
                Method method = toolkit.getClass().getDeclaredMethod(
                        "getMouseInfoPeer", new Class<?>[0]);
                boolean accessibleChanged = false;
                try {
        ...
        if (mouseInfoPeer == null) {
            ...
            int device = mouseInfoPeer
                    .fillPointWithCoords(point != null ? point : new Point());
            GraphicsDevice[] devices = getScreenDevices();
        ...
        if (rval != null) {
            ...
            if (initialScale[0] != bounds.width
                    || initialScale[1] != bounds.height) {
                point.x = Math.round(((float) bounds.width)
                        / ((float) initialScale[0]) * ((float) point.x));
                point.y = Math.round(((float) bounds.height)
                        / ((float) initialScale[1]) * ((float) point.y));
            }
    ...
    public void mouseMove(int x, int y, int ox, int oy) {
        ...
        /*
         * int[] initialScale = new int[2]; Rectangle bounds =
         * _getScreenBounds(device, initialScale);
         * 
         * if (initialScale[0] != bounds.width || initialScale[1] !=
         * bounds.height) { x =
         * Math.round(((float)initialScale[0])/((float)bounds
         * .width)*((float)x)); y =
         * Math.round(((float)initialScale[1])/((float)bounds
         * .height)*((float)y)); }
         */
    ...
    public static void getRGBPixelSlice(int[] src, int srcWidth, int srcHeight,
            int x, int y, int width, int height, int[] dst) {
        assert_(width > 0);
        assert_(height > 0);
    ...
    public boolean getRGBPixels(int x, int y, int width, int height,
            int[] pixels) {
        try {
        ...
        try {
            synchronized (pixelCache) {
                ...
                getRGBPixelSlice(pixelCache[0], this.width, this.height, x, y,
                        width, height, pixels);
                    ...
                    try {
                        ...
                        case 0:
                            getRGBPixelsMethod.invoke(
                                    peer,
                                    new Object[] { Integer.valueOf(r.x),
                                            Integer.valueOf(r.y),
                        ...
                        case 1:
                            getRGBPixelsMethod.invoke(peer, new Object[] {
                                    new Rectangle(r.x, r.y, width, height),
                                    pixelCache[0] });
                            break;
                        case 2:
                            getRGBPixelsMethod.invoke(peer, new Object[] {
                        ...
                        default:
                            getRGBPixelsMethod.invoke(
                                    peer,
                                    new Object[] { getRGBPixelsMethodParam,
                                            Integer.valueOf(r.x),
                                            Integer.valueOf(r.y),
                                            Integer.valueOf(width),
                                            Integer.valueOf(height),
    ...
    public void dispose() {
        ...
        try {
            ...
            try {
                ...
                peer.getClass().getDeclaredMethod("dispose", tailSig)
                        .invoke(peer, (Object[]) tailSig);
            } catch (IllegalArgumentException e) {
                LLog.e(e);
            } catch (SecurityException e) {
                LLog.e(e);
