...
public class ScreenSegmentChanged extends Handle<StateMachine> {
    public static final TaskDispatcher<Integer> segmentFetcher = new TaskDispatcher<Integer>(
            "ScreenSegmentChanged.segmentFetcher");
    ...
    public void handle(final StateMachine stateMachine, Object[] args) {
            ...
            segmentFetcher.dispatch(1, new Runnable() {
                ...
                public void run() {
                    ...
                    if (l_lastSleepTime == null) {
                        ...
                        long refreshMS = (Long) CLIENT_PROPERTIES.MONITOR_SCANNING_PERIOD
                                .getValue();
        ...
        stateMachine.sendEvent(CLIENT_EVENT.ACKNOWLEDGE_NON_SERIAL_EVENT,
                SERVER_EVENT.SCREEN_SEGMENT_CHANGED);
    }
}
