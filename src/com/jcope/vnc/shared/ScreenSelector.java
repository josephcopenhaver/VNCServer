...
public class ScreenSelector {
    ...
    public static GraphicsDevice selectScreen(JFrame parent,
            String promptIfOnlyOneScreen, Integer defaultSelection) {
        ...
        if (devices.length == 1) {
            if (promptIfOnlyOneScreen == null
                    || JCOptionPane.OK_OPTION == JCOptionPane
                            .showConfirmDialog(parent, promptIfOnlyOneScreen)) {
                return devices[0];
            }
            return null;
        }
        ...
        try {
            ...
            for (int i = 0; i < devices.length; i++) {
                ...
                transparentPanel.setBorder(BorderFactory.createEmptyBorder(0,
                        0, 0, 0));
                enumFrame.setContentPane(transparentPanel);

                Rectangle bounds = getScreenBounds(device);
                final Dimension size = new Dimension(bounds.width,
                        bounds.height);
                ...
                JLabel enumLabel = new JLabel(enumMsg) {
                    ...
                    protected void paintComponent(Graphics g) {
                        if (!maximized[0]) {
                            maximized[0] = true;
                            Font maxFont = getMaximumFontForBounds(g, enumMsg,
                                    size, getFont());
                            setFont(maxFont);
                        }
            ...
            Integer selection = (Integer) JCOptionPane.showInputDialog(
                    lastJFrame, "Choose Display Device",
                    "Choose Display Device", JCOptionPane.PLAIN_MESSAGE, null,
                    selections, null);
            ...
            for (JDialog enumFrame : enumFrames) {
                try {
                    ...
                    enumFrame.dispatchEvent(new WindowEvent(enumFrame,
    ...
    private static Font getMaximumFontForBounds(Graphics g, String msg,
        ...
        while ((m = g.getFontMetrics(new Font(fontName, fontStyle, nextSize)))
                .getHeight() < size.height && m.stringWidth(msg) < size.width) {
            curSize = nextSize;
            nextSize++;
            if (nextSize < curSize) {
                break;
