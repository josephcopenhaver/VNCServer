...
public class InputEvent implements Serializable {
    ...
    public static final int MODEX_MASK = (java.awt.event.InputEvent.CTRL_DOWN_MASK
            | java.awt.event.InputEvent.SHIFT_DOWN_MASK
            | java.awt.event.InputEvent.ALT_DOWN_MASK | java.awt.event.InputEvent.META_DOWN_MASK);
    ...
    public InputEvent(INPUT_TYPE type, Object... args) {
        ...
        case WHEEL_SCROLL: {
            ...
            mod = e.getModifiers();
            modex = e.getModifiersEx() & MODEX_MASK;
            // magnitude = e.getPreciseWheelRotation(); // TODO: use this
            // function when on Java 1.7
            ...
            if (type == next.type) {
                if (isFirstCollapse) {
                    ...
                    case MOUSE_UP:
                        ...
                        break;
                    case MOUSE_MOVE:
                        System.arraycopy(next.typeProps, 0, typeProps, 0,
                                typeProps.length);
                        mult++;
                        rval = true;
                        break;
                    case WHEEL_SCROLL:
                        if ((magnitude > 0) == (next.magnitude > 0)
                                && ((magnitude > 0 && magnitude
                                        + next.magnitude > 0) || (magnitude < 0 && magnitude
                                        + next.magnitude < 0))) {
                ...
                if ((type == INPUT_TYPE.KEY_DOWN && next.type == INPUT_TYPE.KEY_UP)
                    ...
                    if (Arrays.equals(typeProps, next.typeProps)) {
                        ...
                        && (next.type == INPUT_TYPE.MOUSE_DOWN
                                || next.type == INPUT_TYPE.MOUSE_UP || next.type == INPUT_TYPE.MOUSE_PRESSED)
                        && sameMousePropertiesExceptPosition(next)) {
                    mult = next.mult;
                    type = next.type;
                    typeProps = next.typeProps;
                    next.typeProps = null; // So old parent can be
                                            // GarbageCollected
                    rval = true;
                }
            }
