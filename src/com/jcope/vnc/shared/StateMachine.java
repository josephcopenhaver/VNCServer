...
public class StateMachine {
    public static enum CONNECTION_STATE {
        INIT, SELECTING_SESSION_TYPE, AUTHENTICATING_INPUT_ENABLED, AUTHENTICATING_VIEW_ONLY, SELECTING_SCREEN_INPUT_ENABLED, SELECTING_SCREEN_VIEW_ONLY, VIEW_WITH_INPUT_ENABLED, VIEW_ONLY
    };

    public static enum CLIENT_EVENT {
        SELECT_SCREEN, GET_SCREEN_SEGMENT, OFFER_INPUT, REQUEST_ALIAS, SEND_CHAT_MSG, ENABLE_ALIAS_MONITOR, ENABLE_CONNECTION_MONITOR, ACKNOWLEDGE_NON_SERIAL_EVENT, GET_CLIPBOARD, CLIPBOARD_CHANGED, SET_CLIPBOARD, SET_SCREEN_MONITOR_PAUSED
    ...
    public static enum SERVER_EVENT {
        ...
        AUTHORIZATION_UPDATE, CLIENT_ALIAS_UPDATE, // Response to client event
                                                    // REQUEST_ALIAS

        NUM_SCREENS_CHANGED, CURSOR_GONE, CURSOR_MOVE, SCREEN_SEGMENT_SIZE_UPDATE, SCREEN_SEGMENT_UPDATE, // Response
                                                                                                            // to
                                                                                                            // client
                                                                                                            // event
                                                                                                            // GET_SCREEN_SEGMENT
        ...
        SCREEN_RESIZED, SCREEN_GONE, CHAT_MSG_TO_ALL, // includes from and text
                                                        // message
        CHAT_MSG_TO_USER, // for debug purposes, should assert only the target
                            // alias
        // gets the message

        // for identifying others listening in
        ALIAS_REGISTERED, ALIAS_UNREGISTERED, ALIAS_DISCONNECTED, ALIAS_CHANGED,
        ...
        CLIPBOARD_CHANGED, // notifies client that server clipboard contents
                            // have changed
        SET_CLIPBOARD, // sends clipboard contents to clients that have
        ...
        public boolean hasMutableArgs() {
            ...
            // nor can they become obsolete whilst waiting
            // to be sent to the data link layer from
        ...
        public boolean isCursor() {
            ...
            switch (this) {
            // These events cannot be flushed when the data out pending queue is
            // idle because
            // they are events sent in an iterative manner and will be followed
            // by another
            ...
            // These events can be flushed when the data out pending queue is
            // idle because
            // they are discrete events not sent in an iterative manner and do
            // not take part
            // in any transaction like event dispatching terminated by a
            // subsequent event
            case SCREEN_SEGMENT_CHANGED:
            case CURSOR_GONE:
            case CURSOR_MOVE:
