...
public class TaskDispatcher<T> extends Thread {
    ...
    private void updateNode(Dispatchable d, Runnable r, Runnable onDestroy,
            Semaphore s) {
    ...
    private void _dispatch(LinkedList q, HashMap<T, Dispatchable> m, T k,
            Runnable r, Runnable onDestroy, Semaphore s) {
    ...
    /**
     * Not very useful unless attempting to do something unique in a dispatched
     * task when the dispatcher could be about to go idle (E.G. if each task is
     * guaranteed to write to a socket and empty, then flush)
     * 
     * @return
     */
