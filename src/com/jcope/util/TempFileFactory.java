    ...
    private static void getSubSystemTempPath() {
        if (subSystemTempPath == null) {
            ...
            synchronized (TempFileFactory.class) {
                ...
                subSystemTempPath = new File(f.getParentFile()
                        .getAbsolutePath(), tmpFileStoreName);
                if ((!subSystemTempPath.exists() && !subSystemTempPath.mkdir())
                        || (subSystemTempPath.exists() && !subSystemTempPath
                                .isDirectory())) {
                    LLog.e(new IOException("Could not create temp directory: "
                            + subSystemTempPath.getAbsolutePath()));
                }
                f.delete();
            }
            sema.release();
