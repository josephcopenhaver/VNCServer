...
public class ClipboardInterface {
    ...
    public static boolean isDataFlavorAvailable(Clipboard clipboard,
            DataFlavor flavor) throws ClipboardBusyException {
        try {
    ...
    public static DataFlavor[] getAvailableDataFlavors(Clipboard clipboard)
            throws ClipboardBusyException {
        try {
    ...
    public static Object getData(Clipboard clipboard, DataFlavor flavor)
            throws ClipboardBusyException, UnsupportedFlavorException,
            IOException {
        try {
    ...
    public static Transferable getContents(Clipboard clipboard, Object requestor)
            throws ClipboardBusyException {
        try {
    ...
    public static void setContents(Clipboard clipboard, Transferable contents,
            ClipboardMonitor clipboardMonitor) throws ClipboardBusyException {
        if (!PLATFORM_IS_MAC) {
            try {
            ...
            if (contents.isDataFlavorSupported(DataFlavor.stringFlavor)) {
                ...
                try {
                    file = TempFileFactory
                            .get("osascript_image_input.", ".png");
                    String escapedFilePath = file.getAbsolutePath()
                            .replaceAll("\\\\", "\\\\")
                            .replaceAll("\"", "\\\"");
                    ...
                    try {
                        file = null;
                        file = TempFileFactory.get(
                                "script_set_clipboard_image.", ".applescript");
                        bw = new BufferedWriter(fw = new FileWriter(file));
                        bw.write(String.format(mac_imgToClipboardApplescript,
                                escapedFilePath));
                        bw.flush();
                        bw.close();

                        Process p = Runtime.getRuntime().exec(
                                new String[] { "osascript",
                                        file.getAbsolutePath() });
                        p.waitFor();
                        tmpFile = null;
                    } finally {
    ...
    private static void getFlavor(ArrayList<Object> pairList, DataFlavor k)
            throws IOException, ClipboardBusyException {
        Object v = null;
    ...
    public static Clipboard getClipboard() {
        ...
        try {
            ...
            LLog.e(new Exception("There is no clipboard here, STOP USING ME!",
                    e), Boolean.TRUE, Boolean.TRUE);
        }
