...
public class NativeBufferedImage {
    ...
    abstract static class Mutator {
        ...
        public void mutate(int idx, int[] pixels, int pixel) {
            ...
            case 0:
                // first three
                pixels[midx] = (pixels[midx] & 0x000000FF)
                        | ((pixel << 8) & 0xFFFFFF00);
                break;
            case 1:
                // last & first two
                pixels[midx] = (pixels[midx] & 0xFFFFFF00)
                        | ((pixel >> 16) & 0x000000FF);
                midx++;
                pixels[midx] = (pixels[midx] & 0x0000FFFF)
                        | ((pixel << 16) & 0xFFFF0000);
                break;
            case 2:
                // last two & first
                midx++;
                pixels[midx] = (pixels[midx] & 0xFFFF0000)
                        | ((pixel >> 8) & 0x0000FFFF);
                midx++;
                pixels[midx] = (pixels[midx] & 0x00FFFFFF)
                        | ((pixel << 24) & 0xFF000000);
                break;
            case 3:
                midx += 2;
    ...
    public NativeBufferedImage(int width, int height, int type) {
        ...
        img = config.createCompatibleImage(width, height,
        ...
        case BufferedImage.TYPE_INT_BGR:
            mutator = new Mutator(4) {
                ...
                @Override
                public int mutate(int pixel) {
                    return (pixel & 0x0000FF00) | ((pixel << 16) & 0x00FF0000)
                            | ((pixel >> 16) & 0x000000FF);
                }
        ...
        case BufferedImage.TYPE_4BYTE_ABGR:
            mutator = new Mutator(4) {
                ...
                @Override
                public int mutate(int pixel) {
                    return (pixel & 0xFF00FF00) | ((pixel << 16) & 0x00FF0000)
                            | ((pixel >> 16) & 0x000000FF);
                }
    ...
    public void setRGB(int dstx, int dsty, int[] srcPixels, int srcx, int srcy,
            int srcw, int srch, int srcScanWidth, int srcScanHeight) {
        ...
        int[] dstPixels = ((DataBufferInt) img.getRaster().getDataBuffer())
                .getData();
    ...
    public void fillRGB(int pixelColor) {
        ...
        if (mutator.bytesPerPixel == 4) {
            ...
            int idx = 0;
            int greatest_factor = dstPixels.length
                    - (dstPixels.length % alignedSolidPixels.length);
            while (idx < greatest_factor) {
                System.arraycopy(alignedSolidPixels, 0, dstPixels, idx,
                        alignedSolidPixels.length);
                idx += alignedSolidPixels.length;
            }
            while (idx < dstPixels.length) {
                dstPixels[idx] = alignedSolidPixels[idx
                        % alignedSolidPixels.length];
                idx++;
            }
        }
